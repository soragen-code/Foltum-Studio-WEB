...
// ── d. schemas accept sceneKind "action" ─────────────────────────────────────────────────────────
{
  ...
  const r = sceneReviseSchema.safeParse({ sceneKind: "action", shotType: "low wide", durationSec: 16, locationDesc: "Courtyard", action: "Yara lunges.", dialogue: 'YARA: "Now."', videoPrompt: base.videoPrompt });
  ok(r.success && r.data.sceneKind === "action", "d: sceneReviseSchema accepts optional sceneKind 'action'");
  const r2 = sceneReviseSchema.safeParse({ shotType: "low wide", durationSec: 16, locationDesc: "Courtyard", action: "Yara lunges.", dialogue: 'YARA: "Now."', videoPrompt: base.videoPrompt });
  ok(r2.success && r2.data.sceneKind === undefined, "d: sceneReviseSchema: sceneKind optional");
  ok(!sceneReviseSchema.safeParse({ sceneKind: "narration", shotType: "low wide", durationSec: 16, locationDesc: "Courtyard", action: "Yara lunges.", dialogue: 'YARA: "Now."', videoPrompt: base.videoPrompt }).success, "d: sceneReviseSchema does not allow switching to narration");
