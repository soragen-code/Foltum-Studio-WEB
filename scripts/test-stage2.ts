...
const mk = (n: number) => Array.from({ length: n }, (_, i) => ({ number: i + 1, shotType: "Medium shot", durationSec: 30, locationDesc: "INT — Маяк — ночь", characters: ["Анна"], action: "Анна входит.", dialogue: i % 6 === 0 ? "[NO DIALOGUE]" : talk, videoPrompt: prompt, startState: "Anna stands in the doorway, facing the window, hands empty, wide shot from the corner.", endState: "Anna stands by the window, back to the door, hands on the sill, medium shot from the doorway." }));
...
assert(/CHARACTERS ACT/.test(EVERYDAY_BEHAVIOR_RULE) && /talking heads/.test(EVERYDAY_BEHAVIOR_RULE) && /staged face to face/.test(EVERYDAY_BEHAVIOR_RULE) && /PHYSICAL DRAMA/.test(EVERYDAY_BEHAVIOR_RULE), "EVERYDAY_BEHAVIOR_RULE: physical business & dramatic physical beats staged face to face");
...
assert(sceneReviseSchema.safeParse({ shotType: "Close-up", durationSec: 30, locationDesc: "INT — Маяк — ночь", action: "Анна молчит.", dialogue: talk, videoPrompt: prompt, startState: "Anna stands in the doorway, facing the window, hands empty, wide shot from the corner.", endState: "Anna stands by the window, back to the door, hands on the sill, medium shot from the doorway." }).success, "scene revise schema ok");
assert(!sceneReviseSchema.safeParse({ shotType: "Close-up", durationSec: 40, locationDesc: "x", action: "y", dialogue: talk, videoPrompt: prompt, startState: "Anna stands in the doorway, facing the window, hands empty, wide shot from the corner.", endState: "Anna stands by the window, back to the door, hands on the sill, medium shot from the doorway." }).success, "scene revise rejects 40s");
