/** Stage 46B-1 unit tests (no live API): look cache hash, look-result validator, single full-body front reference per character (Stage 53; fallback imageFront), cap trimming, look-neutral frame prompt. */
...
(async () => {
  ...
  // 4. Stage 53: individuals send EXACTLY ONE photo — the full-body FRONT shot (imageFull); legacy
  //    characters with only imageFront fall back to it; the profile / extras are never sent to video;
  //    order character anchor → location (all angles) → crowds.
  ...
  ok("Anna sends full-body (front never sent)", urls[0] === styledUrl("anna-full") && !urls.includes(styledUrl("anna-front")));
  ok("Mark (no full) falls back to front", urls[1] === styledUrl("mark-front"));
  ok("crowd sends full-body (front never sent)", urls.includes(styledUrl("crowd-full")) && !urls.includes(styledUrl("crowd-front")));
  ...
  // 5. Stage 53 cap trimming: one full-body ref per character never trimmed, crowds trimmed first
