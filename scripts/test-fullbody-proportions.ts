...
import {
  ...
  evaluateProportions, MAX_TORSO_HEADS, MIN_LEGS_RATIO, MAX_HEADS_TALL,
...
import { characterShotPrompt, FULL_BODY_PROPORTIONS_RULE, PROMPT_MAX_CHARS } from "../lib/full-body-prompt";
...
function unit() {
  ...
  assert.match(adult, /NATURAL-size head/, "Stage 52: head is natural size, not 'SMALL ~1/8'");
...
// Stage 55 — the single full-body reference must be head-to-toe, both feet visible, vertical 9:16 and
// full-height (long legs); the guard fails a short / squashed / cropped body and regenerates.
function unitStage55() {
  // framing wording: head-to-toe, both feet visible, tall vertical 9:16, no crop at waist/hips/knees
  assert.match(FULL_BODY_FRAMING, /head-to-toe/i);
  assert.match(FULL_BODY_FRAMING, /both feet/i);
  assert.match(FULL_BODY_FRAMING, /9:16/);
  assert.match(FULL_BODY_FRAMING, /no cropping at the waist, hips or knees/i);
  // proportions: legs about half of the total height, no short/squashed body, not chibi
  assert.match(FULL_BODY_PROPORTIONS, /roughly half the standing height/i);
  assert.match(FULL_BODY_PROPORTIONS, /NO short stubby legs/i);
  assert.match(FULL_BODY_PROPORTIONS, /NOT chibi/i);
  // the AUTO full-body prompt (text-to-image, as generated by the worker and shown to the user) carries the
  // framing, the proportion block AND the proportion rule — head-to-toe, both feet, 9:16, long legs.
  const auto = characterShotPrompt("A woman in her 30s, red hair", "full", "Mara", null, null, false, "face");
  assert.ok(auto.startsWith(FULL_BODY_FRAMING), "auto full-body prompt leads with the full-length framing");
  assert.ok(auto.includes(FULL_BODY_PROPORTIONS), "auto full-body prompt includes the adult proportion block");
  assert.ok(auto.includes(FULL_BODY_PROPORTIONS_RULE), "auto full-body prompt includes the proportion rule");
  assert.match(auto, /both feet/i);
  assert.match(auto, /9:16/);
  assert.match(auto, /roughly half the standing height/i);

  // guard fails a CROPPED body / feet cut off (framing flags), and regenerates
  const ok: FullBodyCheck = { fullBody: true, feetVisible: true, proportionsOk: true, headsTall: 7.3, issues: [] };
  assert.equal(fullBodyPasses(ok), true);
  assert.equal(fullBodyPasses({ ...ok, fullBody: false }), false, "cropped body (fullBody=false) fails");
  assert.equal(fullBodyPasses({ ...ok, feetVisible: false }), false, "feet cut off fails");
  // guard fails a SHORT / SQUASHED body: short-legs finding, stocky/compressed finding, or too few heads tall
  assert.equal(fullBodyPasses({ ...ok, issues: ["short legs"] }), false, "short-legs finding fails");
  assert.equal(fullBodyPasses({ ...ok, issues: ["stocky/compressed body"] }), false, "squashed/compressed body fails");
  assert.equal(fullBodyPasses({ ...ok, headsTall: 5.5 }), false, "short adult (5.5 heads) fails");
  // measured short legs (legsRatio below MIN_LEGS_RATIO) → shortLegs defect → FAIL
  const shortLegs: FullBodyCheck = { ...ok, proportions: { headsTall: 7.0, torsoHeads: 3.0, legsRatio: 0.40, flags: { elongatedTorso: false, shortLegs: false, smallHead: false, inconsistentVolume: false }, notes: "" } };
  const verdict = evaluateProportions(shortLegs.proportions);
  ...
  // Session 16: leg-length emphasis in the prompt — long adult legs roughly half the standing height, a long
  // coat must not shorten them, an explicit negative against short legs. The hard "exactly half / vertical
  // midpoint / high hip line" wording was softened per the character-reference template; foreshortening is now
  // covered in FULL_BODY_FRAMING (camera block), not the proportion block.
  assert.match(FULL_BODY_PROPORTIONS, /roughly half the standing height/i);
  ...
  assert.match(FULL_BODY_PROPORTIONS_RULE, /roughly half the standing height/i);
  assert.match(FULL_BODY_FRAMING, /no perspective foreshortening or compression of the legs/i);
...
// Stage 58 — the composed full-body prompt must stay under the image provider's 4000-char hard limit
// (Seedream returns HTTP 422 above it, which previously nulled the character's full-body photo). The
// Stage 58 de-duplication (a single proportion block instead of two near-identical copies) plus the
// clampPromptToLimit guard keep every full-body prompt — even one with an absurdly long appearance or a
// manual override — comfortably under the limit, while preserving the framing, the adult-age clause and the
// full proportion wording.
function unitStage58() {
  assert.ok(PROMPT_MAX_CHARS <= 4000 && PROMPT_MAX_CHARS >= 3000, "prompt cap sits below the 4000 provider limit");

  // typical adult full-body prompt (chained + adult age): well under the cap and the hard limit
  const typical = characterShotPrompt("A woman in her 30s, red hair", "full", "Mara", null, null, true, "face", null, "32");
  assert.ok(typical.length <= PROMPT_MAX_CHARS, `typical full-body prompt (${typical.length}) is within the cap`);
  assert.ok(typical.length < 4000, `typical full-body prompt (${typical.length}) is under the provider limit`);
  // the proportion block appears exactly ONCE (Stage 58 de-duplication) — no second near-identical copy
  assert.equal(typical.split(FULL_BODY_PROPORTIONS).length, 2, "the proportion block appears exactly once");

  // pathologically long appearance → clamped, still preserving framing + proportions + adult-age clause
  const hugeAppearance = "wearing an extraordinarily elaborate ceremonial outfit with countless ornate details ".repeat(400);
  const big = characterShotPrompt(hugeAppearance, "full", "Max", null, null, true, "face", null, "40");
  assert.ok(big.length <= PROMPT_MAX_CHARS, `clamped huge-appearance prompt (${big.length}) is within the cap`);
  assert.ok(big.length < 4000, `clamped huge-appearance prompt (${big.length}) is under the provider limit`);
  assert.ok(big.startsWith(FULL_BODY_FRAMING), "clamp preserves the leading framing block");
  assert.ok(big.includes(FULL_BODY_PROPORTIONS), "clamp preserves the full proportion block");
  assert.match(big, /fully grown adult/i, "clamp preserves the E005 adult-age clause");

  // pathologically long MANUAL override → same guarantees (override becomes the character description)
  const hugeOverride = "Fully custom manual prompt describing an intricate elaborate costume in exhaustive detail. ".repeat(400);
  const bigOverride = characterShotPrompt("A man in his 40s", "full", "Leo", null, null, true, "face", hugeOverride, "45");
  assert.ok(bigOverride.length <= PROMPT_MAX_CHARS, `clamped huge-override prompt (${bigOverride.length}) is within the cap`);
  assert.ok(bigOverride.length < 4000, `clamped huge-override prompt (${bigOverride.length}) is under the provider limit`);
  assert.ok(bigOverride.includes(FULL_BODY_PROPORTIONS), "clamp preserves the proportion block for an override too");

  console.log("unit stage58: ok");
}
