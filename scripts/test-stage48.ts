...
async function main() {
  ...
  // Stage 53: the character contributes ONLY the full-body front photo (imageFull) — the front portrait,
  // the profile and the extra angle are never sent to video (sending several photos trips Seedance E005).
  ok("refs: full-body front is Image1", built.referenceImages[0] === FULL);
  ok("refs: front portrait never sent", !built.referenceImages.includes(FACE));
  ...
  ok("refs: full-body front is included", built.referenceImages.includes(FULL));
