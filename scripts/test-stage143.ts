...
async function workerFlowCheck(directionJsons: string[], actions: string[]) {
  ...
  try {
    ...
    ok(calls[3].image_input.join() === [ref.Clara, ref.Dmitri, anchor, u('wide'), u('layout')].join() && /SCENE ANCHOR FRAME \(reference image 3\)/.test(calls[3].prompt), 'worker board 4: Clara + Dmitri refs only, anchor named as reference image 3 (S142 priority kept)');
    ok(calls[6].image_input.join() === [ref.Boris, anchor, u('wide'), u('layout')].join() && /SCENE ANCHOR FRAME \(reference image 2\)/.test(calls[6].prompt), 'worker board 7 (action single): Boris ref only, anchor is reference image 2');
    ok(calls[7].image_input.join() === [ref.Boris, ref.Clara, ref.Dmitri, anchor, u('wide'), u('layout')].join(), 'worker board 8 (group exit): the 3 participants, Anna ref absent');
