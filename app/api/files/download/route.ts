 ...
 *
 * Streams ONE reference frame as an attachment (so a tap on "Download" saves the file instead of opening it).
...
export async function GET(request: Request) {
  ...
  if (!frame) return NextResponse.json({ error: "File not found" }, { status: 404 });
  ...
  if (!upstream || !upstream.ok || !upstream.body) return NextResponse.json({ error: "Failed to upload file" }, { status: 502 });
