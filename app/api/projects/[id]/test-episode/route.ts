 ...
 * Stage 40: turns the project into a "Test episode" sandbox — one season, one episode, ONE scene built
...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (prompt.length < TEST_PROMPT_MIN_CHARS) return NextResponse.json({ error: `Scene prompt is too short (minimum ${TEST_PROMPT_MIN_CHARS} characters)` }, { status: 400 });
  if (prompt.length > TEST_PROMPT_MAX_CHARS) return NextResponse.json({ error: `Scene prompt is too long (maximum ${TEST_PROMPT_MAX_CHARS} characters)` }, { status: 400 });
