...
export const maxDuration = 300; // up to 6 characters × 3 FLUX images
...
/**
 * INTERNAL worker: POST /api/ai/workers/character-images
 * Body: { jobId, projectId, characterIds }
 * Generates the 3 reference images per character sequentially, writing each URL
 * to the Character row and updating the GenerationJob progress after every image.
 */
export async function POST(request: Request) {
  ...
  let jobId = "";
  try {
    const { jobId: id, projectId, characterIds } = await request.json();
    jobId = id;
    if (!jobId || !projectId || !Array.isArray(characterIds))
    ...
    for (const char of characters) {
      for (const shot of SHOTS) {
        ...
        try {
          ...
          failed += 1;
          console.error(`[worker] image ${shot} failed for ${char.name}:`, imgErr?.message ?? imgErr);
        }
        done += 1;
    ...
    return NextResponse.json({ ok: true, total, failed });
  } catch (err: any) {
    console.error("[worker] character-images failed:", err);
    if (jobId) await failJob(jobId, err?.message ?? "Image generation failed");
