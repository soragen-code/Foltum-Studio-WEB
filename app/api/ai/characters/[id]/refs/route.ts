...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (char.refLocked) return NextResponse.json({ error: "Референс персонажа зафиксирован — фото нельзя изменить" }, { status: 409 });

    const current = parseUserRefs(char.userRefs);
    if (current.length >= USER_REFS_MAX) return NextResponse.json({ error: "Максимум 4 фото-референса", userRefs: current }, { status: 409 });
    ...
    if (!ext) return NextResponse.json({ error: "Поддерживаются только JPEG, PNG и WebP" }, { status: 415 });
    if (file.size <= 0) return NextResponse.json({ error: "Пустой файл" }, { status: 400 });
    if (file.size > USER_REF_MAX_BYTES) return NextResponse.json({ error: "Файл больше 8 МБ" }, { status: 413 });
    ...
    if (list.length >= USER_REFS_MAX) {
      ...
      return NextResponse.json({ error: "Максимум 4 фото-референса", userRefs: list }, { status: 409 });
    }
    ...
    return NextResponse.json({ error: "Не удалось загрузить фото: " + (err?.message ?? "Unknown error") }, { status: 500 });
  }
}
...
export async function DELETE(request: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ error: "Не удалось удалить фото: " + (err?.message ?? "Unknown error") }, { status: 500 });
  }
}
