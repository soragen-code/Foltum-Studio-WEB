 ...
 * Stage 40: how "Generate all" runs for this episode — all scenes at once (parallel, joined by
...
export async function PATCH(request: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  if (!isChainMode(chainMode)) return NextResponse.json({ error: "Invalid mode: expected parallel or chain" }, { status: 400 });
