...
// It reuses EpisodeView with `view="shotlist"` so all script actions (generate / regenerate / paste your own,
...
export default async function EpisodeScriptPage({ params }: { params: Promise<{ id: string; episodeId: string }> }) {
  ...
  return <EpisodeView view="shotlist" episode={JSON.parse(JSON.stringify(episode))} project={JSON.parse(JSON.stringify(episode.season.project))} siblings={JSON.parse(JSON.stringify(siblings.map(({ script, ...s }) => ({ ...s, hasScript: !!script }))))} credits={user.credits ?? 0} entitlements={entitlements} />
}
