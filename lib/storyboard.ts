...
export interface RawBoard {
  /** One action beat OR one dialogue pair. Dialogue text is ENGLISH (Stage 141). */
...
export function dialogueRepairSystemPrompt(): string {
  return [
    ...
    "You are given the fixed CAST (canonical names) and a list of quoted LINES (already in ENGLISH) that could not be attributed automatically, each with the text immediately before it as context.",
