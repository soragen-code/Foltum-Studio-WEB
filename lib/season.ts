...
export * from "@/lib/prompts/episode-script";
 ...
 * still load and validate — too MANY scenes is only a "soft:" problem, so legacy episodes are never
 * hard-rejected until their story is regenerated under the new rules.
 */
export const EPISODE_MIN_SCENES = 5;
export const EPISODE_MAX_SCENES = 8;
...
/** Stage 166 — a clip's length is VARIABLE and CONTENT-DRIVEN; this is the FLOOR (even a very short beat lasts at least 3 s). */
...
/** Stage 166 — CEILING for one whole episode: the sum of all scene durations must not EXCEED 100 s (1:40); it may be less. */
export const EPISODE_MAX_TOTAL_SECONDS = 100;
/** Stage 166 — FLOOR for one whole episode: the sum of all scene durations should be AT LEAST 70 s (1:10). */
export const EPISODE_MIN_TOTAL_SECONDS = 70;
/** Stage 115 — a clip is NO LONGER a fixed length. Kept only as a legacy derivation reference. */
export const SCENE_FIXED_SECONDS = 10;
/** Stage 166 — hard CEILING of a single clip: no scene may run longer than 15 s (Stage 115 was 10). */
export const SCENE_CLIP_MAX_SECONDS = 15;
/** Stage 166 — fallback clip length when the script model omits durationSec (a sensible mid-range value in [3,15]). */
export const SCENE_DEFAULT_SECONDS = 9;
/**
 * Stage 166 — an episode's scene count is CONTENT-DRIVEN (5–8). This constant is the AUTO-path cap used by
...
/** Stage 45/72 — legacy frame-state SENTENCE bounds, kept EXPORTED for backward compatibility only; the state
 *  size is no longer sentence-counted. Stage 166 — frame states are now a STRUCTURED CHECKLIST (see
 *  STATE_CHECKLIST_TEXT / STATE_CHECKLIST_ITEMS in lib/prompts/episode-script.ts), verified by an LLM critic
 *  (judgeStateChecklist), NOT by regex / sentence counting. */
export const STATE_MIN_SENTENCES = 36;
export const STATE_MAX_SENTENCES = 60;
/** Stage 166 — a state checklist must be AT LEAST 150 words (was ≥450); the critic, not a word count, is the real gate. */
...
export function episodeFootageGivens(description: string | null | undefined): string {
  ...
  if (!f) return "";
  return `\nHARD BEATS (the episode IS these two beats plus its final frame — the FIRST half of the ${EPISODE_MIN_SCENES}–${EPISODE_MAX_SCENES} scenes expands BEAT 1, the SECOND half expands BEAT 2, and the final frame of the LAST scene is the CLIFFHANGER image; do NOT invent events beyond them, only expand them across the scenes with dialogue, blocking, camera and business):\n${SHOT1_BEAT_LABEL} ${f.shot1}\n${SHOT2_BEAT_LABEL} ${f.shot2}\n${FINAL_FRAME_LABEL} ${f.cliffhanger}`;
}
...
export const sceneScriptSchema = z.object({
  number: z.number().int().min(1),
  /**
   * SHORT human-readable scene HEADER in the story language, formatted as "LOCATION — SUB-LOCATION"
   * (the whole place, then the zone within it, e.g. "Квартира Анны — Кухня" / "Anna's apartment — Kitchen"):
   * the readable place name (mirrors the middle of "locationDesc") + the human-readable spot (the story-language
   * rendering of "subLocation"). It is NOT a dramatic beat and NOT a full sentence. Shown as the readable-script
   * heading next to "Scene N". Optional so legacy / manual scripts still validate; when absent the UI derives the
   * header from "locationDesc" + "subLocation" and finally falls back to "Scene N".
   */
  ...
  // Stage 166 — a SILENT scene (max 2 per episode) is valid ONLY if it carries a visualBeat: a short description
  // of what the viewer READS from the image with no dialogue. Optional so non-silent + pre-Stage-166 scenes parse.
  visualBeat: z.string().optional(),
  // Stage 166 — scene 1's HOOK (conflict / threat / question landing in the first ~3 s). Required-for-scene-1 is
  // enforced in validateEpisodeScript (not here) so older scripts without it still parse.
  hook: z.string().optional(),
  // Stage 166 — optional time skip BEFORE this scene (from the season map, Stage 3). Left OPTIONAL (no zod
  // default) so it stays absent in the inferred SceneScript type and every existing object literal that builds a
  // scene still type-checks; normalizeEpisodeScript coerces an absent / invalid value to "none".
  timeSkipBefore: z.enum(TIME_SKIP_VALUES).optional(),
  ...
  /**
   * Sub-location: a SHORT, STABLE machine key (2–5 lowercase words) naming the distinct SPOT within the scene's
   * location where the action happens (season.ts S16), e.g. "by the tall window", "at the front door". Scenes that
   * share the SAME sub-location reuse one generated 9:16 angle reference (cache by locationId + normalized value).
   * Optional so pre-existing scripts still validate; when absent the scene falls back to the base location reference.
   */
  subLocation: z.string().optional(),
  /**
   * 4-ANGLE REFERENCES — OPTIONAL short ENGLISH phrase (max ~14 words) naming the DISTINCTIVE accent camera
   * angle / vantage this scene is shot from inside its key location (e.g. "low angle from the doorway looking in",
   * "high three-quarter over the back stairs toward the window"). It describes ONLY the camera position/height/
   * framing on the CONSTANT location — never new architecture or props. The distinct accent angles collected
   * across a location's scenes drive that location's extra reference plates (up to 4 angles per location, all of
   * the SAME place). Optional so pre-existing scripts still validate; when absent the location falls back to its
   * default shot plan.
   */
...
export const episodeScriptSchema = z.object({
  ...
  // Stage 166 — the single EMOTIONAL PEAK scene (1-based index). Optional so pre-Stage-166 scripts parse;
  // resolvePeakSceneIndex() defaults it to the middle scene when absent / invalid.
  peakSceneIndex: z.coerce.number().int().min(1).optional(),
});
...
export function validateEpisodeScript(script: EpisodeScript, opts: ValidateEpisodeOptions = {}): string[] {
  ...
  // Stage 166 — up to MAX_SILENT_SCENES (2) silent scenes are allowed, BUT a silent scene is valid ONLY if it
  // carries a `visualBeat` (what the viewer reads with no dialogue). Two failure modes:
  //   (a) MORE than MAX_SILENT_SCENES silent scenes → hard (auto) / soft (manual).
  //   (b) a silent scene with NO visualBeat → hard (auto) / soft (manual) — a silent scene must be intentional.
  const silentScenesList = script.scenes.filter((s) => isSilent(s.dialogue) && !isNarration(s));
  const silentScenes = silentScenesList.map((s) => s.number);
  if (silentScenes.length > MAX_SILENT_SCENES) problems.push(`${silentPrefix}silent scene(s) ${silentScenes.join(", ")}: at most ${MAX_SILENT_SCENES} silent scenes allowed per episode`);
  const silentNoBeat = silentScenesList.filter((s) => !((s.visualBeat ?? "").trim())).map((s) => s.number);
  if (silentNoBeat.length) problems.push(`${silentPrefix}silent scene(s) ${silentNoBeat.join(", ")}: a silent scene is valid ONLY with a "visualBeat" (short description of what the viewer reads with no dialogue)`);
  ...
  // Stage 166 — HOOK / EXPOSITION / CLIFFHANGER / PEAK checks apply to NEW-STYLE scripts only (5–8 scenes).
  // Legacy episodes (9-scene, etc.) have n > EPISODE_MAX_SCENES and are exempt so they still load / validate.
  if (n >= EPISODE_MIN_SCENES && n <= EPISODE_MAX_SCENES) {
    const s1 = script.scenes[0];
    if (s1) {
      // Scene 1 must carry a HOOK and must NOT open on exposition / a "character enters" beat.
      if (!((s1.hook ?? "").trim())) problems.push(`scene 1: missing "hook" (a conflict / threat / question landing in the first ~3 s)`);
      if (looksLikeExpositionOpening(s1.action ?? "")) problems.push(`scene 1: opening reads like exposition or a "character enters" opening — open IN the conflict, not on backstory or an arrival`);
    }
    // Last scene must end on an explicit unresolved image (the cliffhanger carrier is its endState).
    const last = script.scenes[n - 1];
    if (last && !((last.endState ?? "").trim())) problems.push(`scene ${last.number}: last scene must end on an explicit unresolved image (cliffhanger)`);
    // Exactly ONE emotional peak: a supplied peakSceneIndex must point at a real scene (else advisory only).
    const peak = (script as EpisodeScript & { peakSceneIndex?: number | null }).peakSceneIndex;
    if (typeof peak === "number" && (peak < 1 || peak > n)) problems.push(`soft: peakSceneIndex ${peak} is out of range 1..${n} (defaulting to the middle scene)`);
  }
  ...
  // Stage 45/166 — running-time budget: the whole episode should land in the 70–100 s band. Both bounds are
  // advisory (soft): normalizeEpisodeScript already trims what it can, and it cannot pad speech up to 70 s
  // without inventing dialogue, so an under-length episode is reported, never hard-rejected.
  ...
  else if (n >= EPISODE_MIN_SCENES && n <= EPISODE_MAX_SCENES && total < EPISODE_MIN_TOTAL_SECONDS) problems.push(`soft: episode total ${total}s below ${EPISODE_MIN_TOTAL_SECONDS}s (aim for 70–100 s)`);
...
export function normalizeEpisodeScript(script: EpisodeScript, characters?: CharacterCard[], opts?: { manual?: boolean }): EpisodeScript {
  ...
  const scenes = splitOverlongScenes(script.scenes).slice(0, sceneCap).map((s, i) => {
      ...
      // Stage 166 — carry the new fields through: visualBeat (silent scenes), scene-1 hook, and the per-scene
      // timeSkipBefore (validated as an enum, default "none").
      visualBeat: (s.visualBeat ?? "").trim() || undefined,
      hook: (s.hook ?? "").trim() || undefined,
...
- When a CHARACTERS list is given, use ONLY those names (verbatim; a CROWD group name counts as a character). When NO cast is given ("none defined yet"), INVENT the cast yourself from the synopsis: give every character — leads, family, colleagues, rivals, extras and crowd groups alike — a distinct Western name in Latin letters, introduce each one in the text of the episode where they first appear, and use each name IDENTICALLY (same spelling) in every episode's description and "characters" array. Every episode lists 2–6 characters actually present: the MAIN characters carrying it plus the SUPPORTING characters (family, colleagues, rivals) involved. Across the season EVERY SUPPORTING character appears in at least one episode, MINOR characters and CROWD groups are used where the story plausibly gathers people (family dinners, workplaces, hospitals, streets, court, celebrations).
...
- All text except "locationDesc" is in ${langName(language)}. Character names stay exactly as given (or, when you invent the cast, are Western names in Latin letters — never transliterate or translate them). Original content: never reuse names, plots or lines of existing films/series.`;
}
export function seasonStructureUserPrompt(
  ...
  const prev = previousEpisodes && previousEpisodes.length
    ? `PREVIOUS EPISODES ALREADY WRITTEN (continue the SAME story from here — do NOT repeat, reset or re-introduce the premise; the first NEW episode must open directly from the LAST previous episode's cliffhanger, keeping characters, events and the unbroken cliffhanger chain consistent):\n${previousEpisodes
        ...
        .join("\n\n")}\n\n`
    : "";
  // Steps 3/4 are text-only: when the project has no cast cards yet (the cast is extracted later from the finished
  // scripts on the References step) the CHARACTERS section asks the model to invent and name the cast itself.
  const charactersSection = characters.length
    ? `CHARACTERS (with tiers):\n${charactersBlock(characters)}`
    : `CHARACTERS: (none defined yet — INVENT the cast yourself from the synopsis: a distinct Western name in Latin letters for every character incl. extras and crowd groups; use each name with the same spelling in every episode and list the ones present in each episode's "characters" array)`;
...
6. ДОБАВЛЯЙ СОДЕРЖАТЕЛЬНЫЕ ДИАЛОГИ. Вместо коротких восклицаний — разговор, раскрывающий происходящее. Ключевые реплики 1–3 предложения; короткие реплики — для моментов непосредственной опасности. Через диалог объясняй: цель персонажа, конкретный запрет, причину спора, выбор героя, изменение ситуации. Не пиши «Будь осторожна», если можно назвать запрещённое действие. Персонажи разговаривают друг с другом, а не комментируют фильм зрителю. У каждого своя задача. Диалоги — бо́льшая часть сцены.

7. МАКСИМАЛЬНАЯ ДЕТАЛИЗАЦИЯ КАЖДОЙ СЦЕНЫ. Давай как можно БОЛЬШЕ информации о происходящем — пиши развёрнуто, а не скупо. Для каждой сцены раскрывай:
   • ДЕЙСТВИЕ пошагово, в настоящем времени: каждое наблюдаемое движение и жест, мимику ЧЕРЕЗ действия (прищуривается, сжимает челюсть, отводит взгляд), взаимодействие с конкретными предметами (берёт, ставит, открывает, роняет), изменения в пространстве (дверь открывается, свет гаснет, кто-то входит). Описывай именно то, что видно, физически, удар за ударом / шаг за шагом — не «он злится», а что он делает.
   • КАМЕРУ для каждого кадра явно: где стоит, куда направлена, крупность, высота, движение. Один кадр — одна постановка.
   • ЛОКАЦИЮ и ПОДЛОКАЦИЮ явно: в шапке — одно короткое предложение (место), плюс конкретный спот внутри локации, где играет сцена.
...
Return STRICT JSON: {"visualIdentity": string, "peakSceneIndex": int, "scenes": [{"number": int, "title": string, "shotType": string, "durationSec": int, "locationDesc": string, "subLocation": string, "cameraAngle": string, "characters": [names], "action": string, "sceneKind": "dialogue"|"action", "hook": string, "visualBeat": string, "timeSkipBefore": "none"|"minutes"|"hours"|"days"|"weeks", "dialogue": string${local ? ', "dialogueLocal": string' : ""}, "videoPrompt": string, "presence": string, "entrances": string, "continuesFrom": string, "startState": string, "endState": string}]}. "hook" is REQUIRED on scene 1 (omit or leave empty on later scenes); "visualBeat" is REQUIRED on a silent scene (omit on talking scenes); "timeSkipBefore" defaults to "none"; "peakSceneIndex" is the 1-based number of the single emotional-peak scene.
...
R-CLIFF. ${LAST_SCENE_CLIFFHANGER_RULE}
R-PEAK. ${EMOTIONAL_PEAK_RULE}
...
R4. "videoPrompt" and "visualIdentity" are ENTIRELY in ENGLISH (every one of the 9 lines — never ${L}, even though locationDesc/action are in ${L}). "videoPrompt" consists of EXACTLY these 9 lines, each on its own row, in this order, each starting with its bracket tag:
    ...
    [GAZE]: where each character looks, eye contact and reaction while the other speaks; the character delivering a line is turned to FACE the listener with their face visible, never with their back to that listener or turned away during the line
...
STYLE RULES:
S1. ${PACE_DIRECTION}
S2. SPEAKER FRAMING: talking scenes are built AROUND the characters — every shot in which a line is spoken is a DIALOGUE shot: the base cut is a Medium / Medium-close / Over-the-shoulder or waist-up half-body on the speaker and the reactor, the characters large in frame with the location as a soft background behind; a face CLOSE-UP is allowed and encouraged on an emotional beat (a reaction, a decision, a line that lands). The camera angle on the speaker varies with the staging (profile, three-quarter, over-the-shoulder are all fine), but the character delivering a line is ALWAYS turned to FACE the person they address, their face readable in frame — NEVER speaking with their back to that listener or turned away from them (an over-the-shoulder or from-behind CAMERA is fine as long as the speaker still faces the listener). A Wide / establishing / full-length / high-angle / aerial / group / whole-space shot is NOT used while any character is speaking — it is reserved for a NON-dialogue establishing or action beat only (a new or changed location, an entrance / exit, showing arrangement or the geography of a move), never over a spoken line. Only two or three characters are present and converse — never a crowd around the conversation. Never two characters squared off face to face — stage them naturally in the location (different distances/heights, seated/standing, along a counter or rail, one moving while the other stays).
S7. ${MODERATION_SAFE_RULE}
S8. ${CREATIVE_RULE}
S9. ${LOCATION_PRESENCE_RULE}
S10. ${SCALE_DEPTH_RULE}
S11. ${EVERYDAY_BEHAVIOR_RULE}
S12. DYNAMIC TEXT: the episode ALTERNATES between talk-driven beats and active, physical beats — never a run of static conversations. Every scene carries an EVENT that moves the plot (a decision, a discovery, an arrival, a reversal) and INTERWEAVES DIALOGUE WITH ACTION within the scene — the spoken lines are the backbone that carries the beat while the staging and business run alongside them (a line → a move or reaction → the reply → an action), like a serialized drama, so the script reads lively and cinematic, not like talking heads and not like a silent action reel with a token line. The "action" line names a concrete physical EVENT happening in the scene, not a mood — and when the story has reached that point it names the real dramatic beat too (a collision, a strike, a shove, someone drawing or throwing a weapon, a blow landing, someone falling). MANDATORY: when this episode's logline (or the season arc at this point) promises a battle, a duel, an attack, a chase or any physical confrontation, the episode MUST contain at least one "sceneKind": "action" scene in which that confrontation ACTUALLY HAPPENS on screen, staged per R9 — it is never only talked about, threatened or reported afterwards. ${CONFRONTATION_STAGING_SENTENCE}
S13. ${CONTINUITY_RULE}
...
S3. "locationDesc" is MANDATORY and non-empty for EVERY scene — a MACHINE-READABLE location heading in the EXACT format "INT/EXT — <place> — <time of day>" in ${L}, with SPACE-SURROUNDED em-dashes ( — ) as the only separators (e.g. "ИНТ — Храм у моря — ночь", "EXT — Harbour pier — dawn"): the FIRST token is INT/EXT (ИНТ/НАТ), the MIDDLE is ONE short concrete place name (2–5 words, the specific spot the scene plays in — never a full sentence, never a mood, never re-using the "action" prose), and the LAST token is the time of day. Every scene MUST name its place this way so the backend can parse the place name out; a scene stays in the episode's ONE key location (R8), so most scenes repeat the SAME "locationDesc" verbatim, and only a deliberately shown move to another place (continuesFrom "location-change") gets a different one. "action" (4–6 rich sentences — MAXIMUM observable detail, DETAILED choreography of the whole clip at its real length (${SCENE_MIN_SECONDS}–${SCENE_CLIP_MAX_SECONDS} s) written as ONE continuous beat of unbroken motion that runs until the cut — the action ENDS the clip and never finishes early into a frozen pose or a held stare; what each character physically does while each line is spoken) in ${L} — VIVID, concrete staging written with action verbs, not moods: the characters' movement step by step (идёт медленным шагом, разворачивается, бросается вперёд), their gazes and expressions shown through action (смотрит с ухмылкой, смотрит с яростью, прищуривается, сжимает челюсть), their hands INTERACTING with concrete objects (берёт, ставит, открывает, роняет), any change in the space (дверь открывается, свет гаснет, кто-то входит), and — when the story has reached that point — the real physical / dramatic beats the scene needs (замахнулась, ударила в грудь, толкнула, схватила за ворот, кидает копьё, выхватывает нож, удар достигает цели, кто-то падает). ALSO weave in, in ${L}, the concrete SOURCE OF LIGHT (откуда и какой свет падает), the SIGNIFICANT SOUNDS heard in the beat, and each character's POSITION relative to the others and to the objects (кто ближе к двери, кто у окна, кто стоит, кто сидит). Describe exactly what happens, physically, beat by beat, sparing no observable detail. WEAVE THE CAMERA INTO THE ACTION: the "action" prose also names the concrete CAMERA ANGLE and the characters' POSES / BLOCKING for each beat, folded naturally into the sentences (e.g. "over-the-shoulder on Anna as she leans across the table", "tight on his hands as he grips the letter", "a low three-quarter on Marco as he squares up"). Use real framing words — over-the-shoulder, three-quarter, profile, medium-close, waist-up half-body, a face close-up on an emotional beat — together with the body posture (leaning in, turning away, rising to their feet, hands clenched). HARD CONSTRAINT: whenever a character is SPEAKING a line, that beat is framed ONLY as a dialogue shot — over-the-shoulder, three-quarter, profile, medium / medium-close or waist-up half-body on the speaker and reactor, the character large in frame — NEVER a wide / establishing / full-length / aerial / whole-space shot over a spoken line (a wide shot is reserved for a NON-dialogue establishing or action beat: a new location, an entrance / exit, or the geography of a move).
S3b. "title" (SCENE HEADER = LOCATION — SUB-LOCATION): for EVERY scene fill a SHORT human-readable header in ${L}, formatted EXACTLY as "<LOCATION> — <SUB-LOCATION>" with a single SPACE-SURROUNDED em-dash ( — ) as the only separator (e.g. "Квартира Анны — Кухня", "Anna's apartment — Kitchen", "Дом у моря — Терраса"). The LOCATION is the WHOLE place the scene happens in (the human place name — the same place named in the middle of "locationDesc", NOT the INT/EXT token and NOT the time of day); the SUB-LOCATION is the ZONE / spot within it — the human-readable, story-language rendering of the ENGLISH machine key "subLocation" (e.g. subLocation "at the kitchen table" → "Кухня" / "У кухонного стола"). It is a place header shown next to "Scene N" in the readable script, NOT a dramatic beat, NOT a plot line and NOT a full sentence: it must NEVER describe what happens, never end with punctuation, and stay to a few words on each side of the dash. Scenes that share the SAME location + sub-location repeat the SAME header verbatim (mirroring how "subLocation" repeats); when a scene truly has no distinct zone, use just the location name without the dash.
...
S17. ACCENT CAMERA ANGLE (drives the location's reference plates): for EVERY scene fill the ENGLISH field "cameraAngle" — ONE short phrase (max ~14 words) naming the DISTINCTIVE camera angle / vantage this scene is shot from INSIDE the constant key location: its position, height and framing described only through the location's own fixed architecture and set objects (e.g. "low angle from the doorway looking into the depth", "high three-quarter from the gallery over the main floor", "eye-level from behind the counter facing the entrance"). It names NO new furniture and changes NO geometry (R8/S15 hold); it is orthogonal to "subLocation" (the spot) and "region" (the vantage phrase) — "cameraAngle" is specifically the camera SET-UP. Aim to VARY it scene to scene so the episode covers the location from several distinct angles (the distinct accent angles across the location's scenes are what its extra reference plates are generated from — up to 4 plates of the SAME place). Always fill it; on a continuous seam it may still differ (R10 already requires a new camera each seam).
S18. CLARITY & CONCISENESS: write TIGHT, CLEAR prose. Dialogue lines are lean and purposeful — every line reveals, decides or escalates (R3); cut filler, hedging and repetition. In "action"/videoPrompt name concrete beats plainly and ONCE — do not restate the same movement, mood or object in different words, and do not pad sentences to look richer. Favour short, direct sentences with strong verbs over long clause-chains. Keep each field to what a reader needs to shoot the beat: clear over ornate, concrete over abstract, never verbose.
...
export function sceneLocationName(locationDesc?: string | null): string {
  ...
  if (parts.length <= 1) return stripIntExt(t); // no separators — the whole text is the place name
  if (parts.length >= 3) {
    let start = 0;
  ...
  // exactly 2 parts
  return SCENE_LOC_INT_EXT_RE.test(parts[0]) ? stripIntExt(parts[1]) : stripIntExt(parts[0]);
}
...
${DIRECTING_RULES}
MINIMAL-CHANGE / VERBATIM EDIT: apply the author's instruction as LITERALLY and NARROWLY as possible. Change ONLY what the instruction explicitly asks for; keep every other field and every existing wording EXACTLY as given, word for word — do NOT rephrase, polish, shorten, reorder, generalize or "improve" any text the instruction did not target. Preserve the author's exact wording, tone and level of explicitness: NEVER soften, sanitize, euphemize, tone down, abstract away or make any beat safer, vaguer or more polite than the author wrote it — write the requested action DIRECTLY and concretely. NEVER add meta-commentary, self-censorship notes or explanations of what you changed or avoided (no "instead of X, the scene keeps Y", no "to stay tasteful …" phrasing): the output fields contain ONLY the in-world scene content itself, nothing about your editing choices.
Return STRICT JSON: {"sceneKind": "dialogue"|"action", "shotType": string, "durationSec": int, "locationDesc": "INT/EXT — place — time" (${L}), "subLocation": string, "action": string (${L}, 3–4 sentences, a bit more detailed — concrete beat-by-beat staging written as ONE continuous clip-length beat of unbroken motion that runs until the cut, never a page), "dialogue": string${local ? ', "dialogueLocal": string' : ""}, "videoPrompt": string, "presence": string, "entrances": string, "continuesFrom": string, "startState": string, "endState": string}.
"subLocation" is a SHORT, STABLE machine key (2–5 lowercase words, no punctuation) naming the distinct SPOT within the scene's location (e.g. "by the tall window", "at the front door") — name ONLY the spot, never re-describe the location; keep the SAME wording verbatim when the spot is unchanged, and change it only when the action moves to a different spot. Keep the given scene's "subLocation" unchanged unless the instruction moves the action to a different spot.
