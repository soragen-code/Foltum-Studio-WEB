...
export function episodeScriptSystemPrompt(language: IdeaLanguage, episodeNumber = 1): string {
  ...
  const narrationRule = isFirst
    ...
    : `\nR7. OPENING NARRATION (MANDATORY — scene 1 is the EPISODE INTRO): scene 1 opens the episode like a series intro — an off-screen NARRATOR voice-over over wide establishing shots that re-establishes the world and briefly recaps / sets up the backstory (it may recall the catastrophe or event that led here) before the drama resumes. Set "sceneKind": "narration", put 2–4 sentences of English narration (recap / backstory / context) in "voiceover"${local ? ` and its ${L} translation in "voiceoverLocal"` : ""}, and set "dialogue": "[NO DIALOGUE]" (there is NO on-camera talking in scene 1). Its "videoPrompt" is ATMOSPHERIC ESTABLISHING B-ROLL under the narration — ONLY wide/establishing/observational shots of the location and world, NO talking heads, NO character mouths moving, NO lip-sync, NO dialogue close-ups; it still contains all 9 [..] lines (characters may appear in the distance doing ordinary things but NOT speaking). From scene 2 onward the episode is normal on-camera dialogue. Every OTHER scene is "sceneKind": "dialogue" (or "action" when it is a fight — see R9).`;
