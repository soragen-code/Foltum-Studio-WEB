...
export const sceneScriptSchema = z.object({
  number: z.number().int().min(1),
  /**
   * SHORT human-readable scene title (2–6 words) that captures the beat of the scene (e.g. "The Challenge",
   * "A Broken Promise") — NOT a location description and NOT a full sentence. Shown as the readable-script
   * heading next to "Scene N". Optional so legacy / manual scripts still validate; when absent the UI falls
   * back to "Scene N".
   */
...
S3b. "title" (SCENE TITLE): for EVERY scene fill a SHORT human-readable title of 2–6 words in ${L} that captures the DRAMATIC BEAT of the scene — the thing that happens or turns in it (e.g. "The Challenge", "A Broken Promise", "The Offer", "She Says No"). It is a headline shown next to "Scene N" in the readable script, NOT a location line and NOT a full sentence: it must NEVER re-describe the place (that is "locationDesc"), never dump the setting, never end with punctuation, and never run longer than ~6 words. Make each episode's titles distinct from one another.
