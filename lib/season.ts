...
R3. A talking scene = 2–3 lines where characters answer each other, ${TALK_MIN_WORDS}–${TALK_MAX_WORDS} spoken words IN TOTAL (a 15-second clip). Not less than ${TALK_MIN_WORDS} words — short one-liners like "Я должна узнать правду." are REJECTED; if only one character is in the shot, they speak a ${TALK_MIN_WORDS}–${TALK_MAX_WORDS}-word monologue (to themselves, on the phone, to a photo). One line per row, format: NAME (tone cue): "line". Tone cues like (шёпотом), (резко), (сдерживая слёзы).
    Example of a correct talking scene (${TALK_MIN_WORDS}+ words):
    АННА (тихо): "Ты знал, что он не вернётся, и всё равно отправил лодку?"
    ВИКТОР (не глядя): "Я отправил лодку, потому что иначе мы бы потеряли обоих. Ты это понимаешь."
...
Before answering, check: scenes count 10–15; silent scenes ≤ 2; each talking scene ≥ ${TALK_MIN_WORDS} spoken words; every videoPrompt has all 9 tags including [CHARACTER].`;
}
