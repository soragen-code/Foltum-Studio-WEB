...
- INTERIORS ARE ALLOWED (Stage 2 rework): pick whatever mix of interiors and exteriors the STORY actually needs — an intimate drama can live mostly indoors (kitchens, offices, bedrooms, corridors, cars). There is NO rule that open-sky exteriors must predominate. When a location IS an interior, it MUST still ground a fixed geography: give it a clear region-plate (a named, unchanging spatial layout the camera re-frames but never rebuilds) AND a complete "setInventory" as specified above — these are MANDATORY for every interior so the room stays continuous across shots.
- "dramaticFunction": REQUIRED for every location — one sentence, in the story language, stating WHY this place matters to the drama (what conflict, secret or turning point happens here). A location earns its place by carrying story, not by being scenery.
- Locations must be ORIGINAL: no real landmarks, brands, or existing franchises by name.
- The SET of locations must be DIVERSE: vary the type of place, interior vs exterior, private vs public, intimate vs vast, and the time of day and weather — so the season never feels shot in one spot.`;
