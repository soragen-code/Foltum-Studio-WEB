...
export function buildStoryboardAnimationPrompt(board: AnimationBoard): string {
  ...
  const instructions = styledVisualPrompt([
    ...
    plan ? boardShotContext(plan) : `SCENE CAST CONTEXT: ${(board.characters ?? []).join(", ")}. Partners remain present even outside the crop; stable screen sides and 180-degree axis, eyelines at the partner, never at camera.`,
