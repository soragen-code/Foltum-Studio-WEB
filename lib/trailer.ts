...
import { sceneScriptSchema, TALK_MIN_SENTENCES, TALK_MAX_SENTENCES, SCENE_MAX_SECONDS, MODERATION_SAFE_RULE, CONTINUITY_RULE, type LocationRef } from "@/lib/season";
...
Return STRICT JSON: {"title": string, "logline": string, "visualIdentity": string, "locationNames": [1-2 names], "scenes": [{"number": int, "shotType": string, "durationSec": int, "locationDesc": string, "characters": [names], "action": string, "dialogue": string, "dialogueLocal": string, "videoPrompt": string, "presence": string, "entrances": string, "continuesFrom": string}]}.
...
T9. ${CONTINUITY_RULE} Across the 3 shots this means: scenes 1→2 stay in LOCATION A, so the characters KEEP the positions and business they held at the end of scene 1 and simply carry on (no silent reset); the move to LOCATION B in scene 3 is SHOWN and motivated — a character leaves the first place and arrives at the second, or the new/third character is shown entering — never an instant jump to a different room with different people already in place. Write these entrances/exits/moves into [BLOCKING], [ACTION] and [TRANSITION]. For every scene also fill: "presence" (who is where at the START, carried over from the previous shot — for scene 1 the opening setup), "entrances" (who enters or leaves during the shot and how, or "none"), "continuesFrom" (scene 1 = "new-sequence"; a scene in the same place as the previous = "same-location-continuation"; a scene reached by a character moving = "character-moves"; the change to LOCATION B = "location-change").
