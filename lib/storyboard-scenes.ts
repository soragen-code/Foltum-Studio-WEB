...
export function scenePlanSystemPrompt(): string {
  return [
    ...
    "For EACH scene you produce EXACTLY TWO keyframes — a START frame (the first beat of the scene's action) and",
    "an END frame (the scene's final beat) — plus ONE continuous motion description of how the shot moves from the",
    ...
    "  posture, expression and eyeline, what they hold, and the framing (medium / close / wide). Keep the set,",
    "  furniture and lighting identical between the two frames; only poses/positions/action differ.",
    "- The startFrame is the FIRST BEAT OF THE SCENE'S ACTION, not a scene-setting intro. The characters in",
    "  `presentAtStart` / onScreen are ALREADY inside the frame and already doing (or just beginning) the scripted",
    "  `startState` action. Never write the startFrame as an establishing shot, a title card, an empty room or",
    "  landscape, or a wide scenery view with nobody in it or with the characters not yet arrived — open ON the",
    "  people and the action. Prefer a medium or close framing that clearly shows the characters, and only choose a",
    "  wide framing when the scripted action genuinely requires it.",
