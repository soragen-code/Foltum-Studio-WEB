...
export function buildReangleRequest(input: ReangleInput) {
  ...
  const noted = supporting.map(r => r.kind === 'location'
    ? { ...r, note: /layout|elevated/i.test(r.note)
  ...
  const prompt = [
    ...
    'Preserve world-space positions and distances of every person and object, poses and motion phase, gaze targets, occupied/empty hands, held items, faces, anatomy, hair, clothing, materials, geometry, doors, windows, lighting direction and shadows in world space. Project that unchanged world from the new camera; screen positions naturally change. Reveal previously occluded surfaces using the identity and wide/layout references. Never mirror, flip, crop, zoom the source as a substitute for a new viewpoint. Do not add/remove people or objects or pose them toward the camera. Image1 overrides any differing pose, wardrobe, light or gaze of the PEOPLE shown in supporting reference plates; but Image1 is NOT the authority for the placement of the fixed furniture or the architecture — for where every fixed object and structure sits, and against which wall, the layout and wide location plates override Image1.',
