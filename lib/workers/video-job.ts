...
// terminal assembly worker. The scene path below (buildScenePrompt) is DEMOTED to a legacy fallback for
// old episodes that have no Shot rows; new episodes generate one Shot at a time (see runShotVideoJob).
...
import { nextSequentialChainScene, nextSequentialShot, chainStopMessage, CHAIN_INSUFFICIENT_CREDITS } from "@/lib/chain-run";
...
export async function runVideoJob(params: VideoJobParams): Promise<void> {
  // Stage 167 — a shot job takes the per-shot path (its own prompt assembly + chain). Everything below
  // is the LEGACY SCENE path, kept as a fallback for old episodes that were broken down into scenes
  // before the shot pipeline existed (they have no Shot rows) and for scene-level regeneration.
...
async function finalizeVideoJob(jobId: string, state: VideoJobState, source: string) {
  // Stage 167 — a shot job finalizes onto the Shot row (per-shot videoUrl + continuity check + shot chain).
 ...
 * assembled from the Shot's own blocks (assembleShotPrompt), NOT buildScenePrompt. Reference images are
...
async function runShotVideoJob(params: VideoJobParams): Promise<void> {
  ...
  try {
    ...
    // Stage 167 (req 2) — prompt from the SHOT blocks, not buildScenePrompt.
